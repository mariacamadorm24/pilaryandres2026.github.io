...
(function () {
  ...
  function startMusic() {
    ...
    if (!attemptPlay()) {
      const poll = setInterval(() => {
        if (attemptPlay()) clearInterval(poll);
      }, 300);
    ...
    if (galleryImages.length > 0) {
      filmstrip.innerHTML = galleryImages
        ...
        <figure class="filmstrip__frame">
          ...
          ${img.caption ? `<figcaption class="filmstrip__caption">${img.caption}</figcaption>` : ""}
        </figure>`
        )
        .join("");
    } else {
  ...
  const ITINERARY_ICONS = {
    church: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M12 2v4M10 4h4"/><path d="M12 6l8 6v10H4V12l8-6z"/><path d="M9 22v-7a3 3 0 016 0v7"/></svg>',
    toast: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M6 3l4 4-6 6a2.8 2.8 0 004 4l6-6 4 4"/><path d="M14 3l6 6"/><path d="M17 3l3 3"/><path d="M11 5l3 3"/></svg>'
  };
  ...
  timelineList.innerHTML = cfg.itinerary
    .map(
      (item, i) => `
      <article class="itinerary-card reveal reveal-delay-${Math.min(i + 1, 3)}">
        <span class="itinerary-card__icon">${ITINERARY_ICONS[item.icon] || ""}</span>
        <p class="itinerary-card__time">${item.title}</p>
        <p class="itinerary-card__title">${item.time}</p>
        <p class="itinerary-card__detail">${item.detail || ""}</p>
      </article>`
    )
    .join("");
