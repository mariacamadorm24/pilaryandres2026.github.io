...
(function () {
  ...
  const muteIconOn = muteBtn ? muteBtn.querySelector(".icon-unmuted") : null;
  const muteIconOff = muteBtn ? muteBtn.querySelector(".icon-muted") : null;
  ...
  function setMuteIcon(isMuted) {
    ...
    if (muteBtn) muteBtn.setAttribute("aria-label", isMuted ? "Activar música" : "Silenciar música");
  }

  function updateProgress() {
    if (!audioEl || !musicFill || !musicHandle || !audioEl.duration) return;
    const pct = Math.min(100, (audioEl.currentTime / audioEl.duration) * 100);
    musicFill.style.width = pct + "%";
    musicHandle.style.left = pct + "%";
  }

  if (audioEl && musicCfg.enabled && musicCfg.file) {
    audioEl.src = musicCfg.file;
    ...
    audioEl.addEventListener("volumechange", () => setMuteIcon(audioEl.muted));
    audioEl.addEventListener("error", () => {
      console.warn("No se pudo cargar el archivo de audio:", musicCfg.file);
      if (musicPlayer) {
        musicPlayer.innerHTML =
          `<p class="music-player__error">No se pudo cargar la canción. Verifica que el archivo esté en "${musicCfg.file}".</p>`;
      }
    });
  }
  ...
  setText("entrance-kicker", cfg.event.entranceKicker);
  ...
  function startMusic() {
    if (!audioEl || !musicCfg.enabled || !musicCfg.file) return;
    if (musicPlayer) musicPlayer.hidden = false;
    if (muteBtn) muteBtn.hidden = false;
    audioEl.play().catch((err) => {
      // El navegador bloqueó el intento automático; el botón de play
      // manual (ligado a este mismo clic de "Abrir invitación" o a un
      // clic posterior) sigue disponible como respaldo.
  ...
  if (musicPlayBtn) {
    musicPlayBtn.addEventListener("click", () => {
      if (!audioEl) return;
      if (audioEl.paused) {
        audioEl.play().catch((err) => console.warn("No se pudo reproducir:", err));
      } else {
        audioEl.pause();
      }
    });
    ...
    if (galleryImages.length > 0) {
      filmstrip.innerHTML = galleryImages
        ...
        <figure class="filmstrip__frame">
          <img src="${img.src}" alt="${img.alt || ""}" loading="lazy" style="object-position: center ${img.position || "center"};" />
        </figure>`
        )
        .join("");
    } else {
  ...
  const welcomeSignature = document.getElementById("welcome-signature");
  if (welcomeSignature) {
    welcomeSignature.innerHTML =
      `${cfg.couple.nameOne} <span class="amp-glyph">${cfg.couple.ampersand}</span> ${cfg.couple.nameTwo}`;
  }
  ...
  function setCardPhoto(id, photo, altText) {
    const el = document.getElementById(id);
    if (!el) return;
    if (photo) {
      el.src = photo;
  ...
  setCardPhoto("ceremonia-photo", cfg.ceremony.photo, cfg.ceremony.venue);
  ...
  setCardPhoto("recepcion-photo", cfg.reception.photo, cfg.reception.venue);
  ...
  // Vista previa incrustada: un solo Pin (imagen) por referencia, widget oficial de Pinterest.
  const referencePins = cfg.dressCode.referencePins || {};
  const pinLinkWomen = document.getElementById("pin-link-women");
  const pinLinkMen = document.getElementById("pin-link-men");
  if (pinLinkWomen) {
    if (referencePins.women) pinLinkWomen.href = referencePins.women;
    else document.getElementById("pin-embed-women").hidden = true;
  }
  if (pinLinkMen) {
    if (referencePins.men) pinLinkMen.href = referencePins.men;
    else document.getElementById("pin-embed-men").hidden = true;
  }
  if (window.PinUtils && (referencePins.women || referencePins.men)) {
    window.PinUtils.build();
  }

  /* ------------------------------ Aviso de regalos -------------------------------- */
  if (cfg.giftsBanner && cfg.giftsBanner.text) {
    setText("gifts-banner-text", cfg.giftsBanner.text);
    const giftsBannerBg = document.getElementById("gifts-banner-bg");
    if (giftsBannerBg && cfg.giftsBanner.photo) {
      giftsBannerBg.style.backgroundImage = `url("${cfg.giftsBanner.photo}")`;
    }
  } else {
    const giftsBanner = document.getElementById("gifts-banner");
    if (giftsBanner) giftsBanner.hidden = true;
  }
  ...
  const ICONS = {
    ...
    notes: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>'
  };
  ...
  infoGrid.innerHTML = cfg.additionalInfo
    .map((item) => {
      ...
      const introHtml = item.intro
        ...
        : "";
      const bodyHtml = hasBullets
        ? `<ul class="info-card__bullets">${item.bullets.map((b) => `<li>${b}</li>`).join("")}</ul>`
